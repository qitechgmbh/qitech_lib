...
impl<S: Scheduler> VfdDevice<S>
{
    ...
    fn resort(&mut self)
    {
        for i in 1..self.scheduled_requests_len 
        {
            let mut j = i;
            while j > 0 &&
                self.scheduled_requests_buf[j].0 > self.scheduled_requests_buf[j - 1].0
            {
    ...
    fn handle_refresh_telemetry(
        &mut self,
        response: Response,
    ) -> Result<(), HandleResponseError>
    {
        use tokio_modbus::Response::ReadInputRegisters;
        use HandleResponseError::{ParseError, InvalidFunctionCode};
...
impl<S: Scheduler> Device<S> for VfdDevice<S>
{
    ...
    fn next_request(&mut self) -> Option<(Request, bool)> 
    {
        ...
        if self.scheduled_requests_len == 0 { return None; }
        ...
        {
            RefreshTelemetry => 
                Some((Request::ReadInputRegisters(0x8, 6), contains_more)),
            SyncConfig => 
                Some((Request::ReadHoldingRegisters(0x2, 4), contains_more)),
            MutateConfig => 
            {
                use std::borrow::Cow;
