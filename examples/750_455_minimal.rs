...
fn main() {
    ...
    println!("750-455 in slot {ai_slot}, starting ramp...");
    ...
    loop {
        ...
        if let Some(current) = ai.get_current(0) {
            println!("AI {:.2} mA", current.get::<milliampere>());
        } else {
            println!("Wirering Error");
        }
    }
}
