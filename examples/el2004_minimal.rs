...
fn main() {
    ...
    for i in 0..eth_control.controller.subdevice_count {
        println!(" - {:?}", eth_control.controller.subdevices[i].get_name().unwrap());
    }
        ...
        if let Some(outputs) = eth_control.app_handle.write_outputs() {
            for subdevice in eth_control.controller.get_subdevices() {
                ...
                if subdevice.vendor == BECKHOFF_VENDOR_ID && subdevice.product_id == EL2004_PRODUCT_ID {
                    ...
                    el2004.output(BitSlice::<u8, Lsb0>::from_slice_mut(subdevice_outputs)).expect("Failed to write Rx PDO");
                }
            }
        }
